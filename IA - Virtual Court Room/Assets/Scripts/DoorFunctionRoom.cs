...
public class DoorFunctionRoom : MonoBehaviour
{
    ...
    private void openAndCloseLeft()
    {
        ...
        {
            if (doorRotCurr.eulerAngles.y > 90.1f) //Tür wird geschlossen 
            {
        ...
        {
            if (doorRotCurr.eulerAngles.y >= 90f && doorRotCurr.eulerAngles.y < 179.9f) //Tür wird geöffnet
            {
                door.Rotate(0f, 0f, doorSpeed * Time.deltaTime);
            }
            else if (doorRotCurr.eulerAngles.y > 180f || door.rotation == Quaternion.Euler(270, 180, 0)) //Tür wird genau zu 180 Grad, auch wenn sie etwas zu weit rotiert ist
            {
                door.rotation = Quaternion.Euler(270, 180, 0);
                doorIsClosed = false;
                canToggleDoor = false;
            }
        }
    ...
    private void openAndCloseRight()
    {
        ...
        if (!doorIsClosed) //Wenn die Tür offen ist (wie zu Beginn)
        {
            if (doorRotCurr.eulerAngles.y < 89.1f && doorRotCurr.eulerAngles.y >= 0f) //Tür wird geschlossen 
            {
                door.Rotate(0f, 0f, doorSpeed * Time.deltaTime);
            }
            else if (doorRotCurr.eulerAngles.y > 90f || door.rotation == Quaternion.Euler(270, 90, 0)) //Tür wird genau zu 0 Grad, auch wenn sie etwas zu weit rotiert ist
            {
                door.rotation = Quaternion.Euler(270, 90, 0);
                doorIsClosed = true;
                canToggleDoor = false;
        ...
        {
            if (doorRotCurr.eulerAngles.y <= 90f && doorRotCurr.eulerAngles.y > 0.1f) //Tür wird geöffnet
            {
                door.Rotate(0f, 0f, -doorSpeed * Time.deltaTime);
            }
            else if (doorRotCurr.eulerAngles.y > 0f && doorRotCurr.eulerAngles.y > 90f || door.rotation == Quaternion.Euler(270, 0, 0))
            {
